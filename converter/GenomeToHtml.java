...
public class GenomeToHtml {
    ...
    public static void showInbrowser(String fileName) {
        ...
        try {
            ...
            // Wait for the process to die (if firefox opens a new tab, that happens instantly)
            process.waitFor();
    ...
    public static String toHtml(String charName, Gene[] genes, boolean printStuff) {
        ...
        for (Gene gene : genes) {
            // Split the name of the genome by Spaces to later detect if it's a color or a binary value
            ...
            if (split.length == 2)
                ...
                splitB = split[1];
            // If there are two words and the second word is either "red", "green" or "blue":
            if (split.length == 2 && (splitB.equalsIgnoreCase("red") || splitB.equalsIgnoreCase("green") || splitB.equalsIgnoreCase("blue"))) {
                ...
                // And then put the value in again, but this time it's names boolean
        ...
        try {
            ...
            for (Map.Entry<String, HashMap<String, Integer>> item : convertedGenes.entrySet()) {
                ...
                pw.println("</td>");
                pw.println("<td style=\"width: 20px; background-color: rgb(" + item.getValue().get("red") + ", " + item.getValue().get("green") + ", " + item.getValue().get("blue") + ");\">" 
                + ((item.getValue().get("boolean") != null) ? (item.getValue().get("boolean") == 2 ? "Yes" : "No" ) : "") + "</td>");
                pw.println("</tr>");
            }
            pw.println("</table>");
